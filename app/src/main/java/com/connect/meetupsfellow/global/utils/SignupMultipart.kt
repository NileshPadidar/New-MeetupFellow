...
class SignupMultipart(url: URL) {
  ...
  */
     ...
     */
    @Throws(IOException::class)
    fun upload(onFileUploadedListener: OnSignupFileUploadedListener) {
        ...
        try {
            ...
            if (status == HttpURLConnection.HTTP_OK) {
                ...
                LogManager.logger.i(ArchitectureApp.instance!!.tag, "Sign_Response_is ::: $response")
                httpConnection.disconnect()
                onFileUploadedListener?.onFileUploadingSuccess(response)
            }/* else if (status == HttpURLConnection.HTTP_CREATED){
                onFileUploadedListener?.onFileUploadingFailed("Profile Error")
            }*/
            else {
              /*  val reader = BufferedReader(
                    InputStreamReader(
                ...
                val response = reader.use(BufferedReader::readText)
                LogManager.logger.i(ArchitectureApp.instance!!.tag, "Sign_Response_is_multipart::: $response")
                onFileUploadedListener?.onFileUploadingFailed(response)*/

                val errorStream = httpConnection.errorStream
                if (errorStream != null) {
                    val reader = BufferedReader(InputStreamReader(errorStream))
                    val response = reader.use(BufferedReader::readText)
                    LogManager.logger.i(ArchitectureApp.instance!!.tag, "Sign_Response_is_multipart::: $response")
                    onFileUploadedListener?.onFileUploadingFailed("UserName is already taken")
                } else {
                    LogManager.logger.e(ArchitectureApp.instance!!.tag, "email is null")
                    onFileUploadedListener?.onFileUploadingFailed("Email is already taken")
                }
            }
...
interface OnSignupFileUploadedListener {
