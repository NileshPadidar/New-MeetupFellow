...
import com.connect.meetupsfellow.mvp.model.activity.SearchModel
...
class SearchPresenter(internal var mView: SearchConnector.RequiredViewOps) :
    SearchConnector.PresenterOps,
    SearchConnector.RequiredPresenterOps {
    ...
    internal var mModel: SearchConnector.ModelOps = SearchModel(this@SearchPresenter)

    init {
        ArchitectureApp.component!!.inject(this@SearchPresenter)
    }

    private lateinit var requestFeed: RequestFeed

    override fun addSearchObject(requestFeed: RequestFeed) {
    ...
    override fun callRetrofit(type: ConstantsApi) {
        ...
        when (Connected.isConnected()) {
            false -> {
              //  returnMessage(ArchitectureApp.instance!!.getString(R.string.internet_not_connected))
                Log.e("SearchPresenter","internet_not_connected")
                return
            }
        ...
        when (type) {
            ConstantsApi.SEARCH -> callForSearch(type)
            ConstantsApi.SEARCH_EVENTS -> callForSearchEvents(type)
            ConstantsApi.FAVORITE_USER -> callForSearchUser(type)
            ConstantsApi.FAVORITE_EVENT -> callForSearchEvent(type)
            else -> returnMessage(ArchitectureApp.instance!!.getString(R.string.invalid_request))
        }
    }

    private fun callForSearch(type: ConstantsApi) {
        try {
            val search = apiConnect.api.search(sharedPreferencesUtil.loginToken(), requestFeed)
    ...
    private fun callForSearchEvents(type: ConstantsApi) {
        try {
            val search =
                apiConnect.api.searchEvents(sharedPreferencesUtil.loginToken(), requestFeed)
    ...
    private fun callForSearchUser(type: ConstantsApi) {
        try {
            val search =
                apiConnect.api.favoriteUser(sharedPreferencesUtil.loginToken(), requestFeed)
    ...
    private fun callForSearchEvent(type: ConstantsApi) {
        try {
            val search =
                apiConnect.api.favoriteEvent(sharedPreferencesUtil.loginToken(), requestFeed)
