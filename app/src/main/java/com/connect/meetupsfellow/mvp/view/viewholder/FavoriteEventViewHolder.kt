...
class FavoriteEventViewHolder(
    private val binding: ItemFeedBinding, private val clickListener: RecyclerViewClickListener
    ...
    init {
        ...
        ArchitectureApp.component!!.inject(this@FavoriteEventViewHolder)

        if (null != binding.ivEventImage.layoutParams) {
            binding.ivEventImage.layoutParams.height = (DEVICE_HEIGHT / 3)
        }

        itemView.setOnClickListener {
            itemView.context.startActivity(Intent(Constants.Intent.EventDetails).putExtras(Bundle().apply {
                putString(
                    Constants.IntentDataKeys.EventDetails, Gson().toJson(responseEventFeeds.apply {
                        meliked = true
                    })
                )
            }))
    ...
    internal fun bind(responseEventFeeds: ResponseEventFeeds) {
        ...
        LogManager.logger.i(
            ArchitectureApp.instance!!.tag, "People is : ${responseEventFeeds.interestedUsers.size}"
        )
        ...
        when {
            ...
            responseEventFeeds.interestedUsers.size > 3 -> {
                ...
                binding.tvMorePeople.text = itemView.context.getString(
                    ...
                    (responseEventFeeds.interestedUserCount - 2).toString()
                )
            }
        }
    }
    ...
    private fun removeNotAvailableUser() {
        ...
        if (index > -1) {
            LogManager.logger.i(ArchitectureApp.instance!!.tag, "People is Removed  : $index")
            LogManager.logger.i(
                ArchitectureApp.instance!!.tag,
                "People is Removed  : ${Gson().toJson(responseEventFeeds.interestedUsers[index])}"
            )
